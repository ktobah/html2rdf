...
public class SaveRDFN3TRIPLE implements Runnable{
    ...
    public SaveRDFN3TRIPLE(Model model, String base){
        this.model = model;
        this.base = base;
    }
    ...
    public void run() {
        EventQueue.invokeLater(new Runnable() {
            public void run() {
                ...
                choose.setApproveButtonText("Export");
                choose.setDialogType(JFileChooser.SAVE_DIALOG);
                choose.setDialogTitle("Export as N3-TRIPLE (One statement per line, plus QName conversion)");
                choose.setApproveButtonToolTipText("Export the extracted RDF triples");
                choose.addChoosableFileFilter(new FilterRDFN3TRIPLE());
                int decision = choose.showDialog(Interface.getJXFrame(), "Export");
                if (decision == choose.APPROVE_OPTION) {
                    FileOutputStream file;
                    RDFWriter writer = model.getWriter("N3-TRIPLE");
                    try {
                    ...
                    try {
                        if(fileName.contains(".n3")) file = new FileOutputStream(fileName);
                        else file = new FileOutputStream(fileName+".n3");
                        model.setNsPrefix(Base.getBasePrefix(), base);
                        writer.write(model, file, base); //"N3-TRIPLE"
                        //writer.setProperty("abbrevBaseURI", "false");
...
class FilterRDFN3TRIPLE extends FileFilter {
    public final static String rdf = "n3";
